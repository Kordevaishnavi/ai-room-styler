...
import { bufferToGenerativePart } from "@/lib/utils";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const selectedStyleName = styleData?.name || "Modern";

    // Define base prompts for styles (for Gemini)
    const baseStylePrompts: { [key: string]: string } = {
      Industrial: "sophisticated Industrial loft style, exposed brick walls, raw concrete floors, black metal ductwork on the ceiling, reclaimed wood and dark, matte steel furniture, Edison bulb pendants, oversized, black-framed factory-style windows, charcoal, deep brown, and warm grays color palette.",
      Minimalist: "serene Minimalist style, clean, uninterrupted lines, clutter-free space, monochromatic neutral palette of soft whites, light grays, beige, light oak wood, linen curtains, soft wool rug, abundant, soft natural light, functional furniture with simple, elegant geometric forms.",
      Rustic: "cozy modern Rustic aesthetic, like a chic mountain cabin, heavy, warm-toned wood beams on the ceiling, wide-plank hardwood floors, natural fieldstone accent wall or fireplace, comfortable, oversized pieces upholstered in earthy textiles like genuine leather, wool, flannel, warm and layered lighting.",
      Scandinavian: "Scandinavian (Scandi) design, bright, airy, flooded with natural light, crisp white walls with soft gray and pastel accents, light-colored woods like pale oak or ash flooring and furniture, sheepskin throws, wool blankets, plush rugs, several live, green potted plants.",
      Bohemian: "vibrant Bohemian (Boho) sanctuary, eclectic, layered, relaxed, warm, earthy tones, rich, colorful textiles like a Moroccan rug, macrame wall hangings, patterned throw pillows, abundance of thriving houseplants, vintage finds and natural rattan furniture, soft, ambient light from lanterns or string lights.",
      Modern: "sleek Mid-Century Modern style, clean lines, organic curves, low profile furniture, warm walnut wood, polished chrome, glass materials, sophisticated color palette with a neutral base and bold accent colors like teal or burnt orange, smooth and uncluttered surfaces, large-scale abstract art, strong connection to the outdoors with large windows.",
    };
    
    // --- Gemini as "Super Prompt Engineer" ---
    let finalPromptForImageGenerator = `A high-quality, photorealistic interior design image, professional photography, cinematic lighting, ultra-detailed. Redesign this room to embody a ${selectedStyleName} style. Incorporate specific elements like: ${baseStylePrompts[selectedStyleName]}. Ensure all objects and furniture are practical and visually appealing.`;
    
    // Get public URL for the before image (for Gemini analysis)
    const { data: urlData } = supabaseAdmin.storage
      .from("images")
      .getPublicUrl(beforePath);
      
    if (urlData?.publicUrl && process.env.GEMINI_API_KEY) {
      try {
        // Fetch the user's original image from Supabase to send to Gemini
        const userImageResponse = await fetch(urlData.publicUrl);
        const userImageArrayBuffer = await userImageResponse.arrayBuffer();
        const userImageBuffer = Buffer.from(userImageArrayBuffer);
        const userImageMimeType = userImageResponse.headers.get('content-type') || 'image/png';

        const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY!);
        const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });

        const imagePart = await bufferToGenerativePart(userImageBuffer, userImageMimeType);

        // Ask Gemini to refine the prompt based on the image
        const geminiPrompt = `Given this room image and the desired style: '${selectedStyleName}', generate an extremely detailed and vivid positive prompt for an advanced text-to-image AI model. Focus on interior design elements, materials, textures, lighting, furniture types, and overall atmosphere for the '${selectedStyleName}' style. Also, suggest a good negative prompt to avoid common imperfections for this style. Format your response as a JSON object with 'positive_prompt' and 'negative_prompt' keys.
        
        Desired Style Description: ${baseStylePrompts[selectedStyleName]}
        
        Example Output Format:
        {
          "positive_prompt": "photorealistic, hyper-detailed, award-winning interior design photo, cozy living room in a Scandinavian style, light oak wood, sheepskin rug, large windows...",
          "negative_prompt": "blurry, low quality, empty, cluttered, unrealistic, dark, dull colors..."
        }`;

        console.log("Sending image and prompt to Gemini for refinement...");
        ...
        try {
            ...
            if (parsedGeminiResponse.positive_prompt) {
                finalPromptForImageGenerator = parsedGeminiResponse.positive_prompt;
                console.log("Using Gemini-enhanced prompt:", finalPromptForImageGenerator);
            }
        } catch (jsonError) {
            console.warn("Could not parse Gemini's JSON response, using fallback prompt.", jsonError);
        }

      } catch (geminiError) {
          console.error("Error using Gemini as prompt engineer:", geminiError);
          // Continue with the base prompt if Gemini fails
      }
    } else {
      if (!process.env.GEMINI_API_KEY) {
        console.warn("GEMINI_API_KEY not found, using fallback prompt generation");
      }
      if (!urlData?.publicUrl) {
        console.warn("Could not get public URL for image, using fallback prompt generation");
      }
    }

    const prompt = finalPromptForImageGenerator; // Use Gemini's enhanced prompt
